...
# get file
wget -O /etc/openvpn.zip "https://raw.githubusercontent.com/pandavpn03/dash/master/private.zip"
cd /etc/
unzip openvpn.zip
cd
wget -O /var/var.zip "http://dashxhris-dev.ml/autoscript/var.zip"
cd /var/
unzip var.zip
cd
...
visible_hostname TaskForce
cache_mgr TaskForceDEV"| sudo tee /etc/squid/squid.conf	
...
wget -O /etc/stunnel/stunnel.conf "http://dashxhris-dev.ml/autoscript/stunnel.conf"
wget -O /etc/stunnel/stunnel.pem "http://dashxhris-dev.ml/autoscript/stunnel.pem"
chown nobody:nobody /var/run/stunnel
wget -O /etc/rc.d/init.d/stunnel "http://dashxhris-dev.ml/autoscript/stunnel"
...
wget -O /etc/init.d/dropbear "http://dashxhris-dev.ml/autoscript/dropbear"

#get connection
crontab -r
mkdir /usr/sbin/kpn
wget -O /usr/sbin/kpn/connection.php "http://dashxhris-dev.ml/autoscript/privateconnection.sh"
...
#       Setup by: TASKFORCE VPN DEVELOPER   #
#      Modified by: TASKFORCE VPN DEVELOPER #
